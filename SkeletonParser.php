...
class SkeletonParser
{
    ...
    static $skinAttributes = [
            ...
            $fileName, $outputType, $outputMode,
     ...
     * @param string $dir
     */
    static function setViewsDir($dir)
    {
        self::$viewsDir = $dir;
    }
     ...
     * @param string $dir
     */
    static function setSamplesDir($dir)
    {
        self::$samplesDir = $dir;
    }
     ...
     * @param string $dir
     */
    static function setSamplesVariable($variable)
    {
        self::$samplesVariable = $variable;
    }
    ...
    {
        if(file_exists(self::$viewsDir . self::$fileName . self::FILE_EXTENSION))
            return preg_replace(self::PARSE_COMMENTS, '', file_get_contents(self::$viewsDir . self::$fileName . self::FILE_EXTENSION));
        else {
    ...
    private static function fetchPartial($content)
    {
        ...
        for($partials = 0; $partials < $countPartial; ++$partials) {
            if(file_exists(self::$viewsDir . $matchPartial[1][$partials] . self::FILE_EXTENSION)) {
                $partialView = preg_replace(self::PARSE_COMMENTS, '', file_get_contents(self::$viewsDir . $matchPartial[1][$partials] . self::FILE_EXTENSION));
    ...
    private static function fetchParent($content)
    {
        ...
        if(isset($matchParent[1])) {
            ...
            if(file_exists(self::$viewsDir . $matchParent[1] . self::FILE_EXTENSION)) {
                $parentView = preg_replace(self::PARSE_COMMENTS, '', file_get_contents(self::$viewsDir . $matchParent[1] . self::FILE_EXTENSION));
    ...
    static function buildTemplate(SkinElement $node)
    {
        ...
        if(!$node->toParse || $node->tag === 'PLAINTEXT') {
            ...
            if(0 === strpos('PLAINTEXT', strtoupper(ltrim($node->input))))
                $node->start = '';
            $node->end = '';
    ...
    private static function prepareOutputNode(\SkinElement $node)
    {
        ...
        if(isset($skinAttributes['sksample']))
            self::prepareSample($node, $skinAttributes['sksample']);
     ...
     * @return string
     */
    private static function prepareVariableBlocks($node, $code)
    {
    ...
    private static function getChildrenInputs(&$sampleContent, SkinElement $node, $toParse)
    {
        ...
        $input = self::setSpacesByLevel($node->level - $levelZero) . $node->input;
        for($i = 0; $i < $countAttributes; ++$i) {
        ...
        if(isset($node->children)) {
            foreach($node->children as &$child) {
                self::getChildrenInputs($sampleContent, $child, $toParse);
            }
        }
        if(!$toParse) {
            $node->ignore = true;
    ...
    private static function prepareSample(SkinElement $node, $sampleName)
    {
        $explodedSample = explode(';', $sampleName);
        $fileName = self::$samplesDir . self::$fileName . 'Samples.js';
        $sampleContent = ['tree' => ''];
        $toParse = true;
        ...
        self::getChildrenInputs($sampleContent, $node, $toParse);
        self::$sampleManager[$explodedSample[0]] = $sampleContent['tree'];
    ...
    {
        $fileName = self::$samplesDir . self::$fileName . 'Samples.js';
        file_put_contents($fileName, self::$samplesVariable . ' = ' . json_encode(self::$sampleManager));
    }
}
